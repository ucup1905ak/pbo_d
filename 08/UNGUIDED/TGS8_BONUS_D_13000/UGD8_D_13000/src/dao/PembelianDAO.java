...
public class PembelianDAO implements IDAO<PembelianKendaraan, String> {
    ...
    public void insert(PembelianKendaraan pk) {
        ...
        String sql = "INSERT INTO pembelian_kendaraan "
                + "(id_customer, id_kendaraan, id_kasir, id_paketservis, jumlah, tambahan, metode_pembayaran, tanggal_transaksi) "
                + "VALUES (?, ?, ?, ?, ?, ? , ?, ?)";
        ...
        try {
            ...
            ps.setString(3, Integer.toString(pk.getId_kasir()));
            ps.setString(4, pk.getId_paket_servis());
            ps.setInt(5, pk.getJumlah());
            ps.setString(6, pk.getTambahan());
            ps.setString(7, pk.getMetode_pembayaran());
            ps.setString(8, pk.getTanggal_transaksi().toString());
    ...
    public List<PembelianKendaraan> showData(String query) {
        ...
        String sql = "SELECT "
                ...
                + "JOIN kasir ON (PK.id_kasir = kasir.id_kasir) "
                + "JOIN paket_servis ON (PK.id_paketservis = paket_servis.id_paketservis) "
        ...
        try {
            ...
            if (rs != null) {
                while (rs.next()) {
                    ...
                    KasirE kasir = new KasirE(
                            rs.getInt("id_kasir"),
                            rs.getString("nama_kasir"),
                            rs.getString("shift"),
                            rs.getInt("nomor_loket")
                    );
                    PaketServisE paketServis;
                    if (rs.getString("jenis_paketservis").equalsIgnoreCase("ringan")) {
                        paketServis = new PaketRinganE(rs);
                    } else {
                        paketServis = new PaketLengkapE(rs);
                    }
                    PembelianKendaraan PK = new PembelianKendaraan(
                            ...
                            kasir,
                            paketServis,
                            LocalDate.parse(rs.getString("tanggal_transaksi"), DateTimeFormatter.ofPattern("yyyy-MM-dd"))
                    );
    ...
    public void update(PembelianKendaraan pk, String no_resi) {
        ...
        try {
            ...
            ps.setInt(2, pk.getId_kasir());
            ps.setString(3, pk.getId_paket_servis());
            ps.setInt(4, pk.getJumlah());
            ps.setString(5, pk.getTambahan());
            ps.setString(6, pk.getMetode_pembayaran());
            ps.setString(7, pk.getTanggal_transaksi().toString());
            ps.setInt(8, Integer.parseInt(no_resi));
